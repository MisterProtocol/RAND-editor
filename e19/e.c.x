...
#endif  FILELOCKING         /* (this option doesn't really make sense */
...
startup ()
{
    ...
    /*
     *  we'll be appending .hostname to the tmp files
     */
    hostname[0] = '.';
    gethostname(hostname+1, 128);
    hostname[127] = '\0';
    Block {
	    register char *s;
...
#define PRIV (S_IREAD | S_IWRITE | S_IEXEC)
    Block {
	struct stat statbuf;
/*      if (stat(".",&statbuf) != -1 && access (".", R_OK|W_OK|X_OK) >= 0 && */
	if (stat(".",&statbuf) != -1 && access (".", 7) >= 0 &&
		(userid != 0 || statbuf.st_uid == 0)) {
...
#ifdef HOSTTMPNAMES
	    if ( !strncmp (dp->d_name, keytmp, 3)       /* .ek?.hostname */
		    && !strcmp(dp->d_name+4,hostname)
		    && dp->d_name[indv] >= '1'
		    && dp->d_name[indv] <= '9'
...
Reg3 int type;
{
	...
	Block {
	    ...
	    sprintf (recovmsg, "%s/%s", ETC, RECOVERMSG);
