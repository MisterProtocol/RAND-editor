...
La_stream::la_zbreak ()
{
    ...
    return la_break (BRK_REAL, &d1fsd, &d2fsd, &lzero
#ifdef LA_BP
	    , &bzero
#endif
	    );
...
La_stream::la_break (
 ...
 noneed:
	...
	*nbytes = 0;
	...
	La_stream nlas;
	if (!la_break (mode, ffsd, lfsd, &lzero
#ifdef LA_BP
		      , &bzero
#endif
	   ))
	...
	if (la_clone (&nlas) == (La_stream *) 0)
	    goto bad1;
	/*la_sdump (&nlas, "");/**/

	/*  seek to end of area to be deleted, break fsd */
	(void) nlas.la_lseek (nlines, 1);
	if (mode == BRK_COPY)
	    tfsd = *ffsd;
	if (!nlas.la_break (mode == BRK_COPY ? BRK_AGAIN : BRK_REAL, ffsd,
	    lfsd, &lzero
#ifdef LA_BP
		  , &bzero
#endif
	...
	*pnlines = nlas.la_lpos - la_lpos;
#ifdef LA_BP
	*nbytes = nlas.la_bpos - la_bpos;
#endif
	(void) nlas.la_close ();
	if (mode == BRK_COPY)
	    *ffsd = tfsd;
