...
Flag puflg;
{
    ...
    sprintf (filename, "%s/%s", etcdir, deffile);
    if (editfile (filename, (Ncols) 0, (Nlines) 0, 0, puflg) <= 0)
    {
	sprintf(filename, "Default file %s/%s gone: notify system admin.",
	  etcdir, deffile);
	mesg (ERRALL + 1, filename);
    }
    else {
	deffn = curfile;
