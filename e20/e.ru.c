...
Flag    fill_troffmode = YES;   /* default: stop filling at ^. if the fill */
				/*   cmd is given with no parms */
...
#ifdef COMMENT
Cmdret
my_filter (whichfilter, closeflg)
    Small   whichfilter;
    Flag    closeflg;
.
...
Flag    closeflg;
{
    ...
    d_put (0);
    loopflags.beep = YES;
    sprintf (filterpath, "%s/%s", etcdir, filters[whichfilter]);
#ifdef BUILTINFILL
    return callines(filters[whichfilter], whichfilter,
			from, number, QADJUST, puflg, closeflg);
#else /* BUILTINFILL */
    return runlines (filters[whichfilter], filterpath,
...
.
    ...
    seeking backwards on the changes file.  It would be nice if we could do
...
static
Cmdret
callines (funcnm, whichfilter, from, number, iqbuf, puflg, closeflg)
 char *funcnm;
 Small   whichfilter;
 Nlines  from;
 Nlines  number;
 ...
 Flag    closeflg;
{
    ...
    /* We are about to write directly onto the end of
     * the changes file.  The FF buffer cache might contain the last
     * block of the changes file, and it might need to be written out, so
     * we must do it now, not after we have written onto the end of the
     * changes file or the new stuff might get clobbered with zeros
    ...
    if ((toutput = fopen (la_cfile, "w")) == NULL)
	    _exit (-1);
    fseek(toutput, chgend, 0);

    la_lseek(curlas, from, 0);                  /* position ... */

    if (whichfilter==CENTERNAMEINDEX)
	centerit(curlas, toutput, number, linewidth);
    else
	filljust (whichfilter==JUSTNAMEINDEX,   /* justify flag */
	      fill_hyphenate,                   /* break on hyphens */
	      curlas,                           /* input plas */
	      toutput,                          /* output FILE * */
